...
// TODO: identify firedancer tasks properly.
	struct task_struct *parent = p->parent;
    if (parent && bpf_strncmp(parent->comm, 10, "firedancer") == 0) {
        return true;
    }

	return false;
	// /* Check if process name starts with "firedancer" */
	// return bpf_strncmp(p->comm, 10, "firedancer") == 0;
}
